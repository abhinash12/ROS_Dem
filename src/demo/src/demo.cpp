...
int main(int argc, char **argv)
{
  ...
  /******************** From here, we are defining and drawing a simple robot **************************/
    ...
    rob.header.frame_id = path.header.frame_id = "map";  //NOTE: this should be "paired" to the frame_id entry in Rviz
    ...
    if(frame_count % 2 == 0 && path.points.size() <= num_slice2)
    {
      geometry_msgs::Point p;

      float angle = slice_index2*2*M_PI/num_slice2;
      slice_index2 ++ ;
      p.x = 4 * cos(angle) - 0.5;  	//some random circular trajectory
      p.y = 4 * sin(angle) + 1.0;
      p.z = 0.05;
  ...
  /******************** To here, we finished displaying our components **************************/
    ...
    //ros spins, force it to refresh/update once
    ros::spinOnce();
